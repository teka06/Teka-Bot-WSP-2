...
global.authFile = `TekaBotSession`
...
mobile: MethodMobile, 
browser: opcion == '1' ? ['TekaBot', 'Edge', '2.0.0'] : methodCodeQR ? ['TekaBot', 'Edge', '2.0.0'] : ['Ubuntu', 'Chrome', '110.0.1587.56'],
auth: {
creds: state.creds,
...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "TekaJadiBot"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
let prekey = []
let directorio = readdirSync("./TekaBotSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-')
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./TekaBotSession/${files}`)
})
} 

function purgeSessionSB() {
try {
const listaDirectorios = readdirSync('./TekaJadiBot/');
let SBprekey = [];
listaDirectorios.forEach(directorio => {
if (statSync(`./TekaJadiBot/${directorio}`).isDirectory()) {
const DSBPreKeys = readdirSync(`./TekaJadiBot/${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-')
})
SBprekey = [...SBprekey, ...DSBPreKeys];
DSBPreKeys.forEach(fileInDir => {
if (fileInDir !== 'creds.json') {
unlinkSync(`./TekaJadiBot/${directorio}/${fileInDir}`)
}})
}})
...
const directories = ['./TekaBotSession/', './TekaJadiBot/']
