...
const smsOwner = () => { return `\`\`\`¡¡ESTE COMANDO SÓLO MI CREADOR LO PUEDE USAR!!\`\`\`` }
const smsMods = () => { return `\`\`\`¡¡ESTE COMANDO SÓLO MODERADORES Y MI CREADOR LO PUEDEN USAR!!\`\`\`\`` }
const smsPremium = () => { return `\`\`\`¡¡ESTE COMANDO SÓLO ESTA DISPONIBLE PARA USUARIOS PREMIUM Y MI CREADOR!! PARA SER PREMIUM COMPRE UN PASE USANDO #pass premium\`\`\`` }
...
const smsBotAdmin = () => { return `\`\`\`¡¡NECESITAS SER ADMIN PARA QUE PUEDAS USAR ESTE COMANDO!!\`\`\`` }
const smsUnreg = () => { return `\`\`\`¡¡NECESITAS ESTAR REGISTRADO(A) PARA USAR ESTE COMANDO, ESCRIBE #verificar PARA REGISTRARTE!!\`\`\`` }
const smsRestrict = () => { return `\`\`\`¡¡ESTE COMANDO ESTÁ RESTRINGIDO POR MI CREADOR!!\`\`\`` }
...
//info-donar.js
const smsMensajeDonar = () => { return `Por el momento no tenemos un modo!!\n\n Siempre podrás apoyar siguiendo las Cuentas Oficiales y Compartiendo el trabajo logrado en ${packname, ig, md, yt}`}
...
const smsContacto2 = () => { return 'Soy Owner de ' + author + ' sí tienes alguna duda me la puedes decir ✌️'}
const smsContacto3 = () => { return '👑 Owner'}
const smsContacto4 = () => { return 'Contacto Oficial de TekaBot '}
const smsContacto5 = () => { return '🐣 ¿Le puedo ayudar en algo?'}
const smsContacto6 = () => { return 'Correo'}
